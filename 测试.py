...
for book_item in soup.select('.book-details'):
    ...
    sales = int(book_item.select_one('.book-stock').get_text(strip=True).replace('售卖数: ', '').replace('本', ''))
    rating = int(book_item.select_one('.book-rating').get_text(strip=True).replace('好评数: ', ''))
